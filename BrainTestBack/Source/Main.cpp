...
{
	using TBrain = CBrain<CNeuronNormal>;
	using TLayer = TBrain::TLayer;
	...
	try
	{
		...
		auto input = [&](void)
		{
			for (auto& i : *Input)
			{
				auto r = rand() % 2;
				if (r == 0)
					i.SetInput(double((rand() % 5) - 5));
				else
					i.SetInput(double((rand() % 5) + 1));
		...
		auto input = [&](void)
		{
			...
			(*Input)[0].SetInput(pnt.X * pnt.X);
			(*Input)[1].SetInput(pnt.Y * pnt.Y);
		};
		...
		for (int r = 0; r < 100000; ++r)
		{
			...
			for (auto& i : *Input)
				i->SetInput(rand() % 2);
			//(*Input)[0].SetInput(0.0);
			//(*Input)[1].SetInput(0.0);
		...
		TBrain b(1.0, 0.03, { 2, 4, 1 }, Input, Output);
		CAverage Loss(100);
		...
		for (int r = 0; r < 10000; ++r)
		{
			...
			[&](const TLayer& Input_, TLayer& Output_)
			{
				if (Input_[0]->GetInput() * Input_[1]->GetInput() < 0.0)
		...
		for (int r = 0; r < 100000; ++r)
		{
			...
			auto rnd = rand() % 4;
			(*Input)[((rnd & 0x1) > 0 ? 1 : 0)].SetInput(1.0);
			(*Input)[2 + ((rnd & 0x2) > 0 ? 1 : 0)].SetInput(1.0);
