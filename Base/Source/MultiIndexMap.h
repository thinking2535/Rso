...
namespace rso::base
{
	template<typename TValue, typename... TMaps>
	struct map_value_type
	{
		using const_iterators = tuple<typename TMaps::const_iterator...>;
		using data_type = pair<const_iterators, TValue>;
	};

	template<typename TValue, typename... TMaps>
	class CMultiIndexMap : public CList<typename map_value_type<TValue, TMaps...>::data_type>
	{
	public:
		using maps = typename tuple<TMaps...>;
		using iterator = typename CList<typename map_value_type<TValue, TMaps...>::data_type>::iterator;
		using const_iterator = typename CList<typename map_value_type<TValue, TMaps...>::data_type>::const_iterator;
		using const_iterators = typename map_value_type<TValue, TMaps...>::const_iterators;
		using CList<typename map_value_type<TValue, TMaps...>::data_type>::new_index;
		using CList<typename map_value_type<TValue, TMaps...>::data_type>::reserve;
		using CList<typename map_value_type<TValue, TMaps...>::data_type>::get;
		using CList<typename map_value_type<TValue, TMaps...>::data_type>::begin;
		using CList<typename map_value_type<TValue, TMaps...>::data_type>::end;
		using CList<typename map_value_type<TValue, TMaps...>::data_type>::last;
		using CList<typename map_value_type<TValue, TMaps...>::data_type>::front;
		using CList<typename map_value_type<TValue, TMaps...>::data_type>::back;
	...
	public:
		...
		iterator emplace_at(std::size_t DataIndex_, const TValue& Value_, const typename TMaps::key_type&... Keys_)
		{
			const_iterators Iterators;
			_emplace<0>(DataIndex_, Iterators, Keys_...);
			return CList<typename map_value_type<TValue, TMaps...>::data_type>::emplace_at(DataIndex_, pair<const_iterators, TValue>(Iterators, Value_));
		}
		iterator emplace(const TValue& Value_, const typename TMaps::key_type&... Keys_)
		{
			return emplace_at(new_index(), Value_, Keys_...);
		}
		bool erase(iterator it_)
		{
			_erase_tuple(index_sequence_for<typename TMaps::key_type...>{}, it_->first);
			return CList<typename map_value_type<TValue, TMaps...>::data_type>::erase(it_);
		}
		...
		void clear(void)
		{
			...
			CList<typename map_value_type<TValue, TMaps...>::data_type>::clear();
		}
	};
}
