...
namespace rso
{
    namespace base
    {
        ...
        public:
            ...
            public:
                const_iterator(CQueue* Queue_, size_t Index_) :
            ...
            {
            public:
                iterator(CQueue* Queue_, size_t Index_) :
            ...
            CQueue(size_t Size_, TArgsEmplace&&... Args_)
            {
                resize(Size_, forward<TArgsEmplace>(Args_)...);
            }
            CQueue(const CQueue& Var_)
            {
                for (auto& it : Var_)
                    emplace(it);
            }
            ...
            CQueue& operator = (const CQueue& Var_)
            {
                this->~CQueue();
                new (this) CQueue(Var_);
            }
            CQueue& operator = (CQueue&& Var_)
            {
                this->~CQueue();
                new (this) CQueue(std::move(Var_));
            }
            ...
            void emplace(TArgsEmplace&&... Args_)
            {
                ...
                if (NewBack == c_Index_Null)
                {
                    ...
                    try
                    {
                        ...
                        try
                        {
                            new(_Nodes.back())TData(forward<TArgsEmplace>(Args_)...);
                        }
                        catch (...)
                        {
                ...
                {
                    new(_Nodes[NewBack])TData(forward<TArgsEmplace>(Args_)...);
            ...
            {
                return const_iterator((CQueue*)this, _Front);
            }
			inline const const_iterator end(void) const
            {
                return const_iterator((CQueue*)this, c_Index_Null);
            }
